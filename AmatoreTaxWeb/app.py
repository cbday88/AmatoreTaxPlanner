...
with st.sidebar:
    ...
    strategy = st.selectbox("Select Tax Strategy", list(strategies.keys()))
    strategy_desc = strategies[strategy]
    strategy_adj = st.number_input("Strategy Deduction Applied ($)", 0, 1_000_000, 10000, 500)

# --- Run Calculations ---
inp_base = Inputs(status=status, wages=wages, sch_c=sch_c, other_income=other, itemized=itemized, s_corp=False)
inp_scen = Inputs(status=status, wages=wages, sch_c=sch_c, other_income=other, itemized=itemized,
...
st.write("---")
st.metric("Projected Savings", f"${base['total_tax'] - scen['total_tax']:,.0f}")
st.metric("Baseline Total Tax", f"${base['total_tax']:,.0f}")
st.metric("Scenario Total Tax", f"${scen['total_tax']:,.0f}")
st.write("---")

# --- PDF Generator ---
def generate_summary_pdf(base, scen, strategy, strategy_adjustment, strategy_description=""):
    buffer = BytesIO()
    ...
    story.append(Paragraph(strategy_description, styles["Normal"]))
...
if st.button("📄 Generate Client PDF Summary"):
    pdf_data = generate_summary_pdf(base, scen, strategy, strategy_adj, strategy_desc)
    st.download_button(
        label="Download Tax Strategy Summary PDF",
        data=pdf_data,
        file_name=f"Tax_Summary_{strategy.replace(' ', '_')}.pdf",
        mime="application/pdf"
    )

st.caption("Amatore & Co © 2025 | Simplified federal-only MVP. Validate complex cases before delivery.")
